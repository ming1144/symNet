...
void symNet::slidingWindowDetect(string& root, string& folder)
{
	...
	for (int m = 0; m < list.size(); m++)
	{
		...
		do
		{
			...
			for (i = 0; i < boxScores.size(); i++)
			{
				...
				for (j = i + 1; j < boxScores.size(); j++)
				{
					...
					{
						if (rightAbove.x >= border_x_min && rightAbove.x <= border_x_max && rightAbove.y >= border_y_min && rightAbove.y <= border_y_max)
						{
							used[j] = true;
							continue;
						}
		...
		cv::imwrite(resultFolder + "/result_merged/" + filename + ".png", drawedImg2);
		drawedImg2.release();
...
{
	cv::Mat origin = cv::imread(root + '/' + imagePath);
	...
	float angle = 0;
	for (int a = 0; a <= 0; a += 1, angle +=5)
	{
		cv::Mat img = imageTransform::imageRotate(origin, angle);
		cv::Mat gradient, angle, LBP;
		LBP = imageTransform::localBinaryPattern(img);
		imageTransform::gradientAndAngle(img, gradient, angle);
		cv::Mat drawedImg = img.clone();
		...
		for (int y = 0; y < img.rows; y += step_h)
		{
			for (int x = 0; x < img.cols; x += step_w)
			{
				...
				cv::Mat interest = img(ROI);
				//cv::Mat interest = gradient(ROI);
				//cv::Mat interest = angle(ROI);
				//cv::Mat interest = LBP(ROI);
				...
				float score_origin = 0, score_gradient = 0 , score_angle = 0;
				...
				for (int i = 0; i < patch_h; i++)
				{
					for (int j = 0; j < patch_w; j++)
					{
						...
						score_origin += classifier.featureCompare(features[i*patch_w + j], features_mirror[i*patch_w + j]);

						if (score_origin + patch_h*patch_w - i*patch_w + j - 1 < threshold * patchNum)
						{
							jump = true;
							patch.release();
							patch_mirror.release();
				...
				for (int i = 0; i < patch_h; i++)
				{
					for (int j = 0; j < patch_w; j++)
					{
						...
						score_gradient += classifier.featureCompare(features[i*patch_w + j], features_mirror[i*patch_w + j]);

						if (score_gradient + patch_h*patch_w - i*patch_w + j - 1 < threshold * patchNum)
						{
							jump = true;
							patch.release();
							patch_mirror.release();
				...
				for (int i = 0; i < patch_h; i++)
				{
					for (int j = 0; j < patch_w; j++)
					{
						...
						score_angle += classifier.featureCompare(features[i*patch_w + j], features_mirror[i*patch_w + j]);

						if (score_angle + patch_h*patch_w - i*patch_w + j - 1 < threshold * patchNum)
						{
						jump = true;
						patch.release();
						patch_mirror.release();
				...
				if (useNMS)
				{
					...
					temp.Point[2] = x + ROI_width / 2;
					temp.Point[3] = y + ROI_height / 2;
					temp.score = score_origin + score_gradient + score_angle;
		...
		if (useNMS)
		{
			...
			cv::imwrite(filename + "/" + filename + "_NMS_" + std::to_string(a) + ".png", img_NMS);
			img_NMS.release();
		}
		...
		for (i = 0; i < boxScores.size(); i++)
		{
			...
			cv::Rect temp(cv::Point(one_bbox_point[0], one_bbox_point[1]), cv::Point(one_bbox_point[2], one_bbox_point[3]));
			cv::Mat tempMat = img(temp);
			cv::imwrite(filename + "/merged/" + filename + "_" + std::to_string(i) + "_" + std::to_string(a) + ".png", tempMat);

			cv::rectangle(drawedImg2, temp, cv::Scalar(0, 0, 255), 1);
		}
		cv::imwrite(filename + "/" + filename + "_merged_" + std::to_string(a) + ".png", drawedImg2);
		drawedImg2.release();
